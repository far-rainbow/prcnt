...
if ($model->functionList==1) {
    ...
    foreach (array_reverse($wait_prcnt) as $wp => $wd) {
        ...
        if ($alertDay == $wp) {
            ...
            print $wp.' ('. $model->dateru(date('l',strtotime($wp))).') wait_prcnt с '.$startHourList[$model->startHourList].' по '.(($startHourList[$model->startHourList]+$periodTypeList[$model->periodTypeList])%24).' ч. --> '.$wd." -- минимальное значение<br>";
            echo '</li>';
        } else {
            echo '<li class="list-group-item-default">';
            print $wp.' ('. $model->dateru(date('l',strtotime($wp))).') wait_prcnt с '.$startHourList[$model->startHourList].' по '.(($startHourList[$model->startHourList]+$periodTypeList[$model->periodTypeList])%24).' ч. --> '.$wd."<br>";
            echo '</li>';
        }
    }
